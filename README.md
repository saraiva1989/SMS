# SMS
concept test of an application that sends SMS and read the SMS sent and received

# how to use

As this project is based on the test, I put the phone number information (which will be sent to SMS) and the fixed message text in the code, to change are the following strings from the Strings.xml file
PhoneNumber
SMSMessage

Remember that to carry out the test has to be in a physical device and must have a SIM SIM (from the telephone operator).
